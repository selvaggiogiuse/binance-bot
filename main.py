"""
Vendi PRO V2 FAST - Tutti indicatori ma con cache anti-blocco
Fix: cache OHLCV 60s e ticker 10s, meno chiamate, timeout rapido
"""
...
BINANCE_BASES=["https://data-api.binance.vision","https://api1.binance.com"]
...
def get_ohlcv_cached(symbol, interval, limit=200):
    key=f"{symbol}_{interval}_{limit}"
    ttl=30 if interval in ("5m","15m") else 60 if interval=="1h" else 180
    with cache_lock:
    ...
    for base in BINANCE_BASES:
        try:
            ...
            r.raise_for_status()
            data=r.json()
            ohlcv=[{"open":float(c[1]),"high":float(c[2]),"low":float(c[3]),"close":float(c[4]),"volume":float(c[5])} for c in data]
            with cache_lock:
                ohlcv_cache[key]=(time.time(), ohlcv)
            return ohlcv
        except: continue
    # fallback cache even if expired
    with cache_lock:
        if key in ohlcv_cache:
            return ohlcv_cache[key][1]
    return None

def get_ticker_cached(symbol):
    with cache_lock:
        if symbol in ticker_cache:
            ts,price=ticker_cache[symbol]
            if time.time()-ts < 10:
                return price
    for base in BINANCE_BASES:
        try:
            ...
            with cache_lock:
                ticker_cache[symbol]=(time.time(), price)
            return price
        except: continue
    with cache_lock:
        if symbol in ticker_cache:
            return ticker_cache[symbol][1]
    return None
...
def calc_adx(highs, lows, closes, period=14):
    if len(closes)<period*2: return 15
    trs=[]; pdm=[]; mdm=[]
    for i in range(1,len(closes)):
        hl=highs[i]-lows[i]; hc=abs(highs[i]-closes[i-1]); lc=abs(lows[i]-closes[i-1])
        trs.append(max(hl,hc,lc))
    ...
    def wilder(arr,p):
        ...
        return s
    sm_tr=wilder(trs,period); sm_p=wilder(pdm,period); sm_m=wilder(mdm,period)
...
def get_all_signals(tf="4H"):
    ...
    for name,sym in SYMBOLS.items():
        ohlcv=get_ohlcv_cached(sym, interval, 200)
        if not ohlcv:
            results[name]={"symbol":sym,"price":0,"rsi":0,"signal":"OFFLINE","conf":0,"trend":"-","reasons":[],"bullish":0,"bearish":0,"adx":0,"vol_ratio":1,"ema50":0,"ema200":0,"bb_up":0,"bb_low":0,"macd":0,"macd_signal":0,"sl":0,"tp":0}
            continue
        price=get_ticker_cached(sym) or ohlcv[-1]["close"]
        higher=get_ohlcv_cached(sym, higher_interval, 200) if higher_interval else None
...
def checker():
    print("Checker V2 FAST >60% ALL TF")
    while True:
        try:
...
@app.route("/api/ping")
def ping(): return jsonify({"ok":True,"history":len(history),"subs":len(subscriptions),"cache":len(ohlcv_cache)})

@app.route("/api/signals")
def sig(): 
    try:
        return jsonify(get_all_signals(request.args.get("tf","4H")))
    except Exception as e:
        return jsonify({"error":str(e),"coins":{},"globale":"ERRORE","tf":request.args.get("tf","4H"),"updated":datetime.now().strftime("%H:%M:%S")}), 200
...
def testp():
    ...
    sent=send_push(f"TEST V2 {d.get('coin','BTC')} {d.get('tf','5m')} >60%", f"Test FAST - {datetime.now().strftime('%H:%M:%S')}", coin=d.get('coin','BTC'), tf=d.get('tf','5m'))
...
<div class=header><div style="display:flex;gap:10px;align-items:center"><div class=logo>V2⚡</div><div><div style="font-weight:800;font-size:14px">Vendi PRO V2 FAST • NO LAG</div><div style="opacity:.85;font-size:10px">RSI+EMA+BB+MACD+VOL+ADX+ATR • Cache 60s • Push ALL >60%</div><div style="opacity:.7;font-size:9px" id=subStatus>Push: verifica...</div></div></div>⚡</div>
<div class=tfs>
...
<div class=global-card style="background:white;border-radius:16px;padding:12px 14px;display:flex;justify-content:space-between;align-items:center;box-shadow:0 4px 20px rgba(0,0,0,.05)"><div><div style="font-size:9px;color:#64748b">GLOBALE</div><div style="font-weight:800;font-size:14px" id=globale>...</div><div style="font-size:10px;color:#64748b" id=globaleSub>TF 4H</div></div><div style="text-align:right"><div style="font-size:9px;color:#64748b">AGGIORNATO</div><div style="font-weight:700;font-size:12px" id=agg>--</div><div style="font-size:9px;color:#10b981">FAST CACHE • PUSH >60%</div></div></div>
<div class=coin-card id=coins><div style="padding:24px;text-align:center;color:#94a3b8">Caricamento FAST...</div></div>
<div class=coin-card style="margin-top:12px"><div style="padding:10px 14px;display:flex;justify-content:space-between;align-items:center;cursor:pointer" onclick="toggleHist()"><div><b style="font-size:13px">📜 Storico V2 FAST >60% TUTTI TF</b><div style="font-size:10px;color:#64748b">5m 15m 1H 4H 1D</div></div><div id=histArrow>▼</div></div><div id=histList style="display:none;padding:0 8px 8px"></div></div>
...
async function subscribePush(){
  try{
    ...
    document.getElementById('subStatus').innerText='Push: ATTIVO '+j.total+' ALL TF >60%'; alert('Push ALL attiva Tot:'+j.total);
...
async function loadTF(tf){
  ...
  document.getElementById('coins').innerHTML='<div style="padding:20px;text-align:center;color:#94a3b8">FAST '+tf+'...</div>';
  try{
    const controller=new AbortController(); const timeout=setTimeout(()=>controller.abort(), 15000);
    const res=await fetch('/api/signals?tf='+tf,{signal:controller.signal}); clearTimeout(timeout);
    const d=await res.json(); lastData=d;
    if(d.error){document.getElementById('coins').innerHTML='<div style="padding:20px;color:#dc2626">Errore: '+d.error+'</div>'; return;}
    ...
    for(let [name,info] of Object.entries(d.coins)){
      ...
      html+=`<div class=coin-row onclick="openDetails('${name}')"><div style="display:flex;gap:8px;align-items:center"><div class="coin-icon ${icon}">${ico}</div><div><b>${name} <span style="font-size:9px;color:#64748b">ADX ${info.adx.toFixed(0)}</span></b><div style="font-size:10px;color:#64748b">RSI ${info.rsi} • x${info.vol_ratio.toFixed(1)} • ${info.trend}</div><div style="font-size:9px;color:#94a3b8">${info.reasons.slice(0,2).join(' • ')}</div></div></div><div style="text-align:right"><span class="badge ${bgFor(info.signal)}">${info.signal} ${info.conf}%</span><div style="font-weight:800;margin-top:2px;font-size:12px">$${info.price.toFixed(2)}</div><div style="font-size:9px;color:#94a3b8">B${info.bullish}/B${info.bearish}</div></div></div>`;
    }
  ...
  }catch(e){document.getElementById('coins').innerHTML='<div style="padding:20px;text-align:center;color:#dc2626">Timeout, riprovo... '+e.message+'</div>'; setTimeout(()=>loadTF(tf),3000);}
}
...
async function openDetails(coin){
  ...
  document.getElementById('mCoin').innerText=coin+' • '+info.symbol; document.getElementById('mPrice').innerText='$'+info.price.toFixed(2)+' • ADX '+info.adx.toFixed(1);
