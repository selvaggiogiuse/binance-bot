...
import os, requests, time, math, json, base64
...
def compute_entry_quality(comp):
    signal = comp["signal"]
    if signal == "FERMO":
        return {"score": 20, "label": "⏳ ASPETTA", "color": "wait", "simple": "Nessun movimento chiaro. Meglio non fare nulla e aspettare ✅ ENTRA.", "checks": []}
    score = 0
    if comp["conf"] >= 70: score += 20
    elif comp["conf"] >= 65: score += 12
    elif comp["conf"] >= 60: score += 5
    if comp["adx"] >= 25: score += 10
    elif comp["adx"] >= 18: score += 5
    if comp["vol_ratio"] >= 1.2: score += 10
    elif comp["vol_ratio"] >= 0.8: score += 5
    if signal == "COMPRA" and comp["st_trend"] == 1 and comp["price"] > comp["st_val"]: score += 15
    elif signal == "VENDI" and comp["st_trend"] == -1 and comp["price"] < comp["st_val"]: score += 15
    if signal == "COMPRA":
        if comp["stoch_k"] < 40 and comp["stoch_k"] > comp["stoch_d"]: score += 10
        elif comp["stoch_k"] < 25: score += 8
        elif comp["stoch_k"] < 60: score += 4
    else:
        if comp["stoch_k"] > 60 and comp["stoch_k"] < comp["stoch_d"]: score += 10
        elif comp["stoch_k"] > 75: score += 8
        elif comp["stoch_k"] > 40: score += 4
    if signal == "COMPRA" and comp["price"] > comp["vwap"]: score += 10
    elif signal == "VENDI" and comp["price"] < comp["vwap"]: score += 10
    if signal == "COMPRA":
        if comp["dist_sup"] < 1.0 and comp["dist_res"] > 0.8: score += 10
        elif comp["dist_sup"] < 2.0: score += 4
    else:
        if comp["dist_res"] < 1.0 and comp["dist_sup"] > 0.8: score += 10
        elif comp["dist_res"] < 2.0: score += 4
    if signal == "COMPRA" and comp["ema50"] > comp["ema200"]: score += 5
    elif signal == "VENDI" and comp["ema50"] < comp["ema200"]: score += 5
    if signal == "COMPRA" and comp["macd"] > comp["macd_signal"]: score += 5
    ...
    if signal == "COMPRA":
        if score >= 70:
            ...
            simple = f"Sta provando a salire, ma non è ancora fortissimo (score {score}%). Meglio aspettare la prossima candela."
        else:
            simple = f"Non è un buon momento per comprare (score {score}%). Trend debole, rischi di perdere fee."
    else:
        if score >= 70:
            ...
            simple = f"Sta provando a scendere, ma non è ancora fortissimo (score {score}%). Meglio aspettare."
        else:
            simple = f"Non è un buon momento per vendere (score {score}%). Sei vicino al supporto, potrebbe rimbalzare."
    ...
    if score >= 70:
        ...
        simple = "Meglio non fare nulla adesso. Aspetta un segnale con ✅ ENTRA. Così non perdi l'1€."

    return {"score": score, "label": label, "color": color, "simple": simple, "checks": []}
...
# --- ICON ROUTES (nuova icona 2) ---
ICON_DIR_CANDIDATES = ["/mnt/data", ".", "/tmp", os.getcwd()]
def find_icon(name):
...
def icon_192():
    p = find_icon("icon-192.png")
    if p: return send_file(p, mimetype="image/png")
    return "", 404
@app.route("/icon-512.png")
def icon_512():
    p = find_icon("icon-512.png")
    if p: return send_file(p, mimetype="image/png")
    return "", 404
@app.route("/apple-touch-icon.png")
def apple_icon():
    p = find_icon("apple-touch-icon.png") or find_icon("icon-192.png")
    if p: return send_file(p, mimetype="image/png")
    return "", 404
@app.route("/favicon.ico")
@app.route("/favicon-32.png")
def favicon():
    p = find_icon("favicon-32.png") or find_icon("icon-192.png")
    if p: return send_file(p, mimetype="image/png")
    return "", 404
@app.route("/icon-1024.png")
def icon_1024():
    p = find_icon("icon-1024.png") or find_icon("icon-512.png")
    if p: return send_file(p, mimetype="image/png")
    return "", 404
...
def manifest():
    return jsonify({
        "name": "Vendi€ PUSH - V10 LITE",
        "short_name": "Vendi€",
        "description": "Trading 1€ con ENTRA/ASPETTA facile per principianti",
        "start_url": "/app?v=10",
        "display": "standalone",
...
@app.route("/api/ping")
def ping(): return jsonify({"ok":True,"msg":"V10 LITE + ICONA 2","time":rome_now().isoformat(),"subs": len(load_subs())})
...
<title>Vendi€ PUSH - V10 LITE</title>
...
<meta name="apple-mobile-web-app-title" content="Vendi€">
...
<div><b style="font-size:18px">Vendi€ PUSH V10 LITE</b><br><small>Modalità Principiante + Icona 2 Dark</small><br><small style="opacity:.7">Tocca per aggiungere a Home - Icona nuova attiva ✅</small></div>
</div>
...
<details style="margin-top:14px"><summary style="font-size:12px;color:#64748b;cursor:pointer">🔧 Dettagli esperto (V9)</summary>
...
async function loadTF(tf){{
  ...
  document.getElementById('coins').innerHTML='<div style="padding:20px;text-align:center">⏳ Carico V10...</div>';
  try{{
...
async function openDetails(coin){{
  ...
  if(info.quality_color=='entra'){{
    big.innerHTML=`<div style="font-size:22px;font-weight:800;color:${{info.signal=='COMPRA'?'#166534':'#991b1b'}}">${{info.quality_label}} - ${{info.signal}}</div><div style="font-size:14px;margin-top:6px">Puoi entrare con 1€ finto</div><div style="font-size:12px;color:#64748b;margin-top:4px">Score ${{info.quality_score}}% • SL $${{info.sl.toFixed(0)}} TP $${{info.tp.toFixed(0)}}</div>`;
    ...
    big.innerHTML=`<div style="font-size:20px;font-weight:800;color:#475569">⏳ ASPETTA</div><div style="font-size:13px;margin-top:6px">Non è il momento giusto</div><div style="font-size:11px;color:#64748b">Meglio aspettare ✅ ENTRA</div>`;
  }}
...
async function loadBacktest(coin, tf){{
  try{{
    ...
    document.getElementById('mWinRateBig').innerHTML=`Ultimi 12: ${{(j.last20||[]).map(x=>x.win?'🟢':'🔴').join('')}}<br>Vinti <b>${{last20}}%</b> • ${{j.wins}}/${{j.total_signals}} totali`;
  }}catch(e){{}}
}}
function closeModal(){{document.getElementById('modal').classList.remove('show')}}
loadTF('1H'); setInterval(()=>loadTF(curTF),30000);
updatePaperBar();
if('serviceWorker' in navigator){{navigator.serviceWorker.register('/sw.js').then(()=>{{console.log('SW V10 icona 2')}});}}
</script>
</body></html>
"""
