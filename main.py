...
VERSION = "V70 ULTIMATE - TUTTI I METODI + V67 + BOS"
COOLDOWN = 600
LAST_TELEGRAM = {}
LAST_ENTRA = {}
STABLE_SECONDS = 180
...
def macd_calc(closes):
    ...
    # signal EMA9 del macd (semplificato)
    return macd_line, macd_line*0.9, macd_line - macd_line*0.9  # macd, signal, hist
...
def analyze_bos(ohlc):
    ...
    diff_h=(last_h-prev_h)/prev_h*100; diff_l=(last_l-prev_l)/prev_l*100
    if hh and hl: return {"type":"BOS BULL HH+HL","signal":"COMPRA","bonus":30,"desc":f"HH {prev_h:.0f}→{last_h:.0f} (+{diff_h:.2f}%) HL {prev_l:.0f}→{last_l:.0f}","last_h":last_h,"prev_h":prev_h,"diff_h":diff_h}
    elif lh and ll: return {"type":"BOS BEAR LH+LL","signal":"VENDI","bonus":30,"desc":f"LH {prev_h:.0f}→{last_h:.0f} ({diff_h:.2f}%) LL {prev_l:.0f}→{last_l:.0f}","last_h":last_h,"prev_h":prev_h,"diff_h":diff_h}
...
def analyze_all_methods(ohlc, ohlc_1h, ohlc_4h=None):
    closes=[c["close"] for c in ohlc]
    highs=[c["high"] for c in ohlc]; lows=[c["low"] for c in ohlc]; vols=[c["volume"] for c in ohlc]
    price=closes[-1]
    methods={}
    # 1. EMA STACK 9/21/50/150/200 - V67 + tuo 50/150
    ...
    if price>ema9 and ema9>ema21 and ema21>ema50 and ema50>ema150 and ema150>ema200:
        methods["EMA"]={"signal":"COMPRA","score":25,"desc":"EMA 9>21>50>150>200 BULL perfetto"}
    ...
    if 50<=rsi<=65:
        methods["RSI"]={"signal":"COMPRA","score":20,"desc":f"RSI {rsi:.0f} 50-65 bull"}
    elif 35<=rsi<=50:
        methods["RSI"]={"signal":"VENDI","score":20,"desc":f"RSI {rsi:.0f} 35-50 bear"}
    elif rsi>75:
        methods["RSI"]={"signal":"VENDI","score":10,"desc":f"RSI {rsi:.0f} ipercomprato → reversal"}
    elif rsi<25:
        methods["RSI"]={"signal":"COMPRA","score":10,"desc":f"RSI {rsi:.0f} ipervenduto → reversal"}
    else:
        methods["RSI"]={"signal":"ASPETTA","score":0,"desc":f"RSI {rsi:.0f} neutro"}
    # 4. MACD
    macd, signal, hist = macd_calc(closes)
    if hist>0 and macd>0:
        methods["MACD"]={"signal":"COMPRA","score":15,"desc":f"MACD {macd:.2f} > signal bullish"}
    elif hist<0 and macd<0:
        methods["MACD"]={"signal":"VENDI","score":15,"desc":f"MACD {macd:.2f} < signal bearish"}
    else:
        methods["MACD"]={"signal":"ASPETTA","score":0,"desc":"MACD piatto"}
    # 5. Bollinger Bands
    sma20, upper, lower = bb_calc(closes,20,2)
    if price < lower:
        methods["BB"]={"signal":"COMPRA","score":15,"desc":f"BB prezzo sotto lower {lower:.0f} → rimbalzo"}
    elif price > upper:
        methods["BB"]={"signal":"VENDI","score":15,"desc":f"BB prezzo sopra upper {upper:.0f} → ritraccio"}
    elif price > sma20:
        methods["BB"]={"signal":"COMPRA","score":5,"desc":"BB sopra SMA20"}
    else:
        methods["BB"]={"signal":"VENDI","score":5,"desc":"BB sotto SMA20"}
    # 6. Stochastic
    stoch=stoch_calc(ohlc,14)
    if 20<=stoch<=40:
        methods["STOCH"]={"signal":"COMPRA","score":10,"desc":f"Stoch {stoch} oversold → long"}
    elif 60<=stoch<=80:
        methods["STOCH"]={"signal":"VENDI","score":10,"desc":f"Stoch {stoch} overbought → short"}
    elif 40<stoch<60:
        methods["STOCH"]={"signal":"ASPETTA","score":0,"desc":f"Stoch {stoch} neutro"}
    else:
        methods["STOCH"]={"signal":"ASPETTA","score":5,"desc":f"Stoch {stoch} estremo"}
    # 7. Volume - V67
    avg_vol=sum(vols[-20:])/20 if len(vols)>=20 else 1
    cur_vol=vols[-1] if vols else 1
    vol_ratio=cur_vol/avg_vol if avg_vol>0 else 1
    if 1.5<=vol_ratio<=5.0:
        methods["VOL"]={"signal":"COMPRA","score":10,"desc":f"Vol x{vol_ratio:.1f} buono (conferma)","vol_ratio":vol_ratio}
    elif vol_ratio>6.0:
        methods["VOL"]={"signal":"ASPETTA","score":-10,"desc":f"Vol x{vol_ratio:.1f} pump/news → stop","vol_ratio":vol_ratio}
    elif vol_ratio<1.0:
        methods["VOL"]={"signal":"ASPETTA","score":-5,"desc":f"Vol x{vol_ratio:.1f} basso → no entrata","vol_ratio":vol_ratio}
    else:
        methods["VOL"]={"signal":"ASPETTA","score":5,"desc":f"Vol x{vol_ratio:.1f} ok","vol_ratio":vol_ratio}
    # 8. Multi Timeframe 1H + 15m - V67
    h1_up=True; h1_rsi=50
    if ohlc_1h and len(ohlc_1h)>=21:
        ...
        if h1_up:
            methods["1H"]={"signal":"COMPRA","score":15,"desc":f"1H UP RSI{h1_rsi:.0f}"}
        else:
            methods["1H"]={"signal":"VENDI","score":15,"desc":f"1H DOWN RSI{h1_rsi:.0f}"}
    else:
    ...
    if swing_highs and swing_lows:
        ...
        if dist_res < 0.5:
            methods["SR"]={"signal":"VENDI","score":15,"desc":f"Vicino resistenza {last_res:.0f} ({dist_res:.2f}%) → short"}
        elif dist_sup < 0.5:
            methods["SR"]={"signal":"COMPRA","score":15,"desc":f"Vicino supporto {last_sup:.0f} ({dist_sup:.2f}%) → long"}
        else:
            methods["SR"]={"signal":"ASPETTA","score":0,"desc":f"SR lontano Sup {last_sup:.0f} Res {last_res:.0f}"}
    else:
    ...
    if len(ohlc)>=3:
        ...
        if last["close"]>last["open"] and prev["close"]<prev["open"] and last["close"]>prev["open"] and last["open"]<prev["close"]:
            ...
            methods["CANDLE"]={"signal":"ASPETTA","score":0,"desc":"No pattern candle"}
    else:
        methods["CANDLE"]={"signal":"ASPETTA","score":0,"desc":"No candle"}
    # 11. VWAP (semplificato: SMA 50 come proxy)
    vwap=sma_calc(closes,50)
    if price>vwap:
        methods["VWAP"]={"signal":"COMPRA","score":10,"desc":f"Prezzo > VWAP {vwap:.0f}"}
    else:
        methods["VWAP"]={"signal":"VENDI","score":10,"desc":f"Prezzo < VWAP {vwap:.0f}"}
    # 12. ATR Volatility - V67
...
def send_tg(coin, tf, signal, conf, price, sl, tp, sl_pct, tp_pct, source, rsi, extra, force=False, is_real=False, methods=None):
    ...
    text=f"""{emoji} *{signal} {coin} {conf}%* ⚡ {tf} V70 ULTIMATE
...
V70: TUTTI I METODI + V67 + BOS tuo cerchi blu"""
    try:
        ...
        if r.status_code==200:
            ...
            TRADE_HISTORY.append({"time":rome_now().isoformat(),"timestamp":time.time(),"expiry":expiry,"coin":coin,"tf":tf,"signal":signal,"entry":price,"sl":sl,"tp":tp,"conf":conf,"mode":RISK_CONFIG["mode"],"result":None,"pnl_pct":0,"auto":True,"adaptive":adaptive,"methods":str(methods)[:300]})
...
def analyze(name, tf, do_tg=False, force_tg=False):
    ...
    try:
        ...
        # Calcolo punti totali separati per COMPRA e VENDI
        compra_score=0; vendi_score=0
        for k,v in methods.items():
            if v["signal"]=="COMPRA": compra_score+=v["score"]
            elif v["signal"]=="VENDI": vendi_score+=v["score"]
        # Decidi segnale finale in base a punteggio
        if compra_score > vendi_score and compra_score >= 40:
            signal="COMPRA"
            conf_base=compra_score
            diff=compra_score-vendi_score
        elif vendi_score > compra_score and vendi_score >= 40:
            signal="VENDI"
            conf_base=vendi_score
            diff=vendi_score-compra_score
        else:
            signal="ASPETTA"
            conf_base=max(compra_score,vendi_score)
            diff=0
        # Normalizza conf a 0-95
        ...
        if signal=="COMPRA":
            ...
            sl=price*0.992; tp=price*1.015; sl_pct=0.8; tp_pct=1.84
        # Extra con tutti i metodi
        bull_list=[]; bear_list=[]
        for k,v in methods.items():
            if v["score"]>0:
                if v["signal"]=="COMPRA": bull_list.append(f"{k}:{v['score']}")
                else: bear_list.append(f"{k}:{v['score']}")
        extra=f"BULL {compra_score} [{','.join(bull_list[:4])}] vs BEAR {vendi_score} [{','.join(bear_list[:4])}] • {methods.get('BOS',{}).get('desc','')} • {methods.get('1H',{}).get('desc','')} • Vol x{vol_ratio:.1f} ATR {atr_pct:.2f}% • Adapt {adaptive}% • V67+V69+10metodi"
        ...
        if conf>=min_conf and vol_ok and bos_ok and signal!="ASPETTA" and (regime_ok or not is_real_mode):
            color="entra"; label=f"ENTRA {signal} BULL{compra_score} vs BEAR{vendi_score}"
        elif conf>=62:
...
def run_backtest(coin="BTC", tf="15m", limit=200):
    ...
    for i in range(60, len(ohlc)-6):
        ...
        # Filtro BOS obbligatorio per coerenza con tuo metodo
...
def ai_market_answer(question, coin="BTC", tf="5m"):
    ...
    if "tutti" in q or "metodi" in q or "ultimate" in q:
        return f"V70 ULTIMATE con TUTTI i metodi + V67 + tuo BOS:\n\nV67 dentro: Adattivo {data.get('adaptive',0)}% + ATR {data.get('atr_pct',0):.2f}% SL*1.5 TP*2.3 R:R 1:2.3 + News Vol>6.5 stop + Sessione 6-23 + Equity ${RISK_CONFIG['equity']:.0f} DD {RISK_CONFIG['drawdown']:.1f}% + EMA 9/21/50\n\n+ TUO BOS cerchi blu: {data.get('bos_type','')} {data.get('bos_desc','')[:80]}\n\n+ ALTRI 10 METODI: EMA50/150/200 + RSI + MACD + BB + Stoch + Vol + 1H + SR + Candle + VWAP\n\nBULL {compra} vs BEAR {vendi} = {sig} {conf}% WR live {wr:.1f}% - Se BULL>>BEAR entra. Tutti votano."
    return f"V70 ULTIMATE {coin} {tf}: ${price:.2f} {sig} {conf}% BULL{compra} vs BEAR{vendi} | BOS {data.get('bos_type','')} | RSI {rsi} | {data.get('extra','')[:120]} | WR {wr:.1f}% Eq ${RISK_CONFIG['equity']:.0f}"

@app.route("/")
def home(): return Response(f"{VERSION} - {rome_now()} - Mode {RISK_CONFIG['mode']} - BULL vs BEAR voting", mimetype="text/plain")
@app.route("/health")
    ...
    try:
        ...
        for name in PAIRS.keys():
            try:
                ...
                if d is None: d={"price":0,"source":"LOADING","signal":"LOADING","conf":0,"quality_color":"loading","quality_label":"CARICO ULTIMATE...","rsi":50,"stoch_k":50,"vol_ratio":1,"sl":0,"tp":0,"sl_pct":0.8,"tp_pct":1.84,"rr":2.3,"spark":[],"extra":"Carico TUTTI I METODI...","h1":"--","is_real":False}
                res[name]=d
                if tr: tg[name]=tr
            except Exception as e:
                import traceback; traceback.print_exc()
                res[name]={"price":0,"source":"ERROR","signal":"ERROR","conf":0,"quality_color":"wait","quality_label":"ERRORE","rsi":50,"stoch_k":50,"vol_ratio":1,"sl":0,"tp":0,"sl_pct":0.8,"tp_pct":1.84,"rr":2.3,"spark":[],"extra":str(e)[:100],"h1":"--","is_real":False}
...
def tg_test():
    r=send_tg("BTC","15m","COMPRA",78,80000,79400,81200,0.7,1.61,"TEST",55,"Test V70 ULTIMATE",force=True,is_real=(RISK_CONFIG["mode"]=="REAL"),methods={})
    return jsonify(r)
...
def force_tg():
    ...
    for name in PAIRS.keys():
        ...
        out[name]=send_tg(name,"15m","COMPRA",78,p,p*0.995,p*1.01,0.5,1.15,"FORCE",55,"Force ULTIMATE",force=True,is_real=(RISK_CONFIG["mode"]=="REAL"),methods={})
    return jsonify(out)
...
<title>V70 ULTIMATE - TUTTI I METODI</title>
<style>
...
<div class="header"><div class="logo">V70</div><div style="flex:1"><div style="font-weight:800">V70 <span style="background:#f59e0b;color:#422006;padding:2px 6px;border-radius:6px;font-size:9px">ULTIMATE 12 METODI</span></div><div style="font-size:9px;color:#94a3b8">V67 + BOS tuo + EMA + RSI + MACD + BB + Stoch + Vol + SR + Candle + VWAP + 1H/4H</div></div><div style="display:flex;gap:6px"><button onclick="openAI()" style="background:#f59e0b;color:#422006;border:none;padding:6px 10px;border-radius:20px;font-size:11px;font-weight:700">🤖 AI</button><button onclick="openRisk()" style="background:#1e293b;color:white;border:1px solid #334155;padding:6px 10px;border-radius:20px;font-size:11px">⚙️ Risk</button></div></div>
<div id="banner" class="banner banner-ultimate">V70 ULTIMATE: 12 METODI CHE VOTANO - V67 (Adattivo 72-86% + ATR SL*1.5 TP*2.3 R:R 1:2.3 + News Vol>6.5 + Sessione 6-23 + Equity+DD) + BOS tuo HH/LH cerchi blu + EMA 9/21/50/150/200 + RSI + MACD + BB + Stoch + Vol + 1H + SR + Candle Engulfing + VWAP</div>
<div id="riskBar" class="riskBar"><span id="riskMode">Mode: DEMO</span><span id="riskCap">Cap: $1000</span><span id="riskWR">WR: 0%</span><span id="riskEquity">Eq: $1000</span><span id="riskAdapt">Adapt: 78%</span><span id="riskScores">BULL vs BEAR</span><span><button onclick="openHistory()" style="background:#22c55e;color:#052e16;border:none;padding:4px 8px;border-radius:10px;font-size:10px;font-weight:800">📓 Diario</button> <button onclick="runBT()" style="background:#f59e0b;color:#422006;border:none;padding:4px 8px;border-radius:10px;font-size:10px;font-weight:800">📊 Backtest 12 metodi</button></span></div>
<div class="tfs"><button id="b5m" onclick="loadTF('5m')">⚡ 5m 12M</button><button id="b15m" class="active" onclick="loadTF('15m')">15m ULTIMATE</button><button id="b1H" onclick="loadTF('1H')">1H 12M</button><button id="b4H" onclick="loadTF('4H')">4H 12M</button><button onclick="loadTF(curTF,true,true)" style="background:#22c55e;color:#052e16">📱 Forza TG</button><button onclick="nuke()" style="background:#dc2626;color:white">💣 NUKE</button></div>
<div id="coins"><div style="padding:20px;text-align:center;color:#94a3b8">Carico V70 ULTIMATE 12 metodi + V67 + tuo BOS...</div></div>
<div id="riskModal" class="modal" onclick="if(event.target==this)closeRisk()"><div class="box"><b>⚙️ Risk V70 ULTIMATE 12 METODI</b><div style="font-size:10px;color:#fde68a;background:#422006;border:1px solid #f59e0b;padding:8px;border-radius:8px;margin:6px 0;white-space:pre-wrap">12 METODI CHE VOTANO (somma punteggi):

V67 dentro:
1. Adattivo 72-86% (sale se WR<50%, scende se >68%)
2. ATR dinamico SL=ATR*1.5 TP=SL*2.3 R:R 1:2.3
3. News filter Vol>6.5 stop
4. Sessione 6-23 no notte
5. Equity + Peak + DD>6% stop
6. EMA 9/21/50 + RSI + Vol + 1H conf

TUO BOS V69:
7. BOS HH/LH cerchi blu - confronto massimi destra vs sinistra

ALTRI 6 METODI NUOVI:
8. EMA 150/200 + MACD + BB + Stoch + SR + Candle Engulfing + VWAP + 4H

Ogni metodo da 0-30 punti. BULL vs BEAR: se BULL 80 vs BEAR 20 → COMPRA 80% conf. Se BULL 40 vs BEAR 45 → ASPETTA.</div><div style="display:grid;gap:10px;margin-top:10px">
<label style="font-size:12px">Modalità<br><select id="rMode" style="width:100%;padding:10px;background:#020617;color:white;border:1px solid #334155;border-radius:10px"><option value="DEMO">🟡 DEMO ULTIMATE 12M</option><option value="REAL">🔴 REAL ULTIMATE 12M</option></select></label>
...
async function nuke(){if(!confirm('NUKE V70 ULTIMATE 12 metodi?'))return;try{let r=await fetch('/api/nuke');alert('✅ NUKE ULTIMATE 12 metodi - V67 + BOS + 10 altri');location.reload();}catch(e){alert(e.message);}}
...
document.getElementById('coins').innerHTML='<div style="padding:20px;text-align:center;color:#94a3b8">⚡ Carico '+tf+' V70 ULTIMATE 12 metodi...</div>';
...
if(d.telegram_results && Object.keys(d.telegram_results).length>0){html+=`<div style="background:#422006;padding:8px 12px;font-size:10px;color:#fde68a;text-align:center">📱 TG ULTIMATE: ${JSON.stringify(d.telegram_results)} | BULL vs BEAR voting</div>`;}
document.getElementById('coins').innerHTML=html;
}catch(e){
clearTimeout(timeout);
document.getElementById('coins').innerHTML='<div style="padding:20px;color:#ef4444;text-align:center">Timeout ULTIMATE<br><button onclick="nuke()" style="margin-top:10px;background:#dc2626;color:white;border:none;padding:10px 20px;border-radius:20px">💣 NUKE</button><br><small>'+e.message+'</small></div>';
}
}
...
function copySLTP(){if(!curCoin||!lastData) return; let info=lastData.coins[curCoin]; let txt=`${curCoin} ${info.price.toFixed(2)} SL ${info.sl.toFixed(2)} TP ${info.tp.toFixed(2)} ULTIMATE BULL${info.compra_score} BEAR${info.vendi_score} BOS ${info.bos_type}`; navigator.clipboard.writeText(txt).then(()=>alert('Copiato ULTIMATE 12 metodi'));}
...
async function sendNow(){if(!curCoin) return; try{let r=await fetch('/api/signals?tf='+curTF+'&telegram=1&force=1'); let j=await r.json(); alert('TG ULTIMATE: '+JSON.stringify(j.telegram_results));}catch(e){alert(e.message);}}
...
function askAboutCoin(){if(!curCoin) return; closeM(); openAI(); document.getElementById('aiInput').value='Tutti i metodi + V67 + BOS su '+curCoin+'?'; sendAI();}
...
async function loadHistory(){try{let r=await fetch('/api/history');let j=await r.json(); document.getElementById('histStats').textContent=`Totale ${j.total} - WIN ${j.wins} - LOSS ${j.losses} - Pending ${j.pending} - WR ${j.winrate}% - PnL ${j.pnl_sum}% - Eq $${j.equity} - V70 ULTIMATE 12M`; let list=document.getElementById('histList');let html=''; j.history.slice().reverse().forEach((t,i)=>{let col=t.result=='WIN'?'#22c55e':t.result=='LOSS'?'#ef4444':'#facc15'; html+=`<div style="display:flex;justify-content:space-between;padding:8px;border-bottom:1px solid #1e293b;font-size:11px"><div><b>🤖 ${t.coin} ${t.tf} ${t.signal} ${t.conf}%</b> $${t.entry?.toFixed(2)} → ${t.result?`$${(t.result=='WIN'?t.tp:t.sl).toFixed(2)}`:'...'}<br><span style="color:#94a3b8">${t.time.slice(11,19)} ${t.mode} PnL ${t.pnl_pct?.toFixed(2)}%</span></div><div style="text-align:right"><span style="color:${col};font-weight:800">${t.result||'APERTO'}</span></div></div>`;}); list.innerHTML=html||'Nessun trade ULTIMATE';}catch(e){alert(e.message);}}
...
async function runBT(){let coin=curCoin||'BTC';let tf=curTF; document.getElementById('btStats').textContent='Carico backtest ULTIMATE 12 metodi '+coin+' '+tf+'...'; document.getElementById('btList').innerHTML=''; document.getElementById('btModal').classList.add('show'); try{let r=await fetch(`/api/backtest?coin=${coin}&tf=${tf}`); let j=await r.json(); if(!j.ok){document.getElementById('btStats').textContent='Errore: '+j.error; return;} document.getElementById('btStats').textContent=`V70 ULTIMATE ${j.coin} ${j.tf}: ${j.total} trade, ${j.wins} WIN, ${j.losses} LOSS, WR ${j.winrate}% - 12 metodi + V67 + BOS`; let html=''; j.trades.reverse().forEach(t=>{let col=t.result=='WIN'?'#22c55e':'#ef4444'; html+=`<div style="display:flex;justify-content:space-between;padding:8px;border-bottom:1px solid #1e293b;font-size:12px"><div><b>${t.signal}</b> $${t.entry.toFixed(2)} BULL${t.compra} BEAR${t.vendi}<br><span style="font-size:10px;color:#94a3b8">📅 ${t.time} ${t.bos} Conf ${t.conf}</span></div><div style="text-align:right"><span style="color:${col};font-weight:800">${t.result}</span></div></div>`;}); document.getElementById('btList').innerHTML=html||'Nessun trade ULTIMATE';}catch(e){document.getElementById('btStats').textContent='Errore: '+e.message;}}
