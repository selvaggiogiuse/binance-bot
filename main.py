...
TELEGRAM_MIN_CONF = 85  # base, poi adattivo
PAIRS = {"BTC": "BTCUSDT", "ETH": "ETHUSDT", "ORO": "PAXGUSDT"}
VERSION = "V67 PERFECT - ADATTIVA + ATR + NEWS FILTER + EQUITY"
COOLDOWN = 600
LAST_TELEGRAM = {}
LAST_ENTRA = {}
STABLE_SECONDS = 180
TRADE_HISTORY = []
RISK_CONFIG = {"mode": "DEMO", "capital": 1000.0, "risk_pct": 1.0, "max_trades_day": 3, "max_losses_row": 3, "daily_trades": 0, "daily_losses_row": 0, "last_day": str(date.today()), "equity": 1000.0, "peak": 1000.0, "drawdown": 0.0}
OHLC_CACHE = {}
ADAPTIVE_CONF = 85  # si alza/abbassa da solo in base a WR
...
def get_adaptive_threshold():
    ...
    # Se ultimi 20 trade WR <50%, alza soglia a 88%, se WR >65% abbassa a 82% per più segnali
    if len(TRADE_HISTORY) < 10:
        return ADAPTIVE_CONF
    recent = [t for t in TRADE_HISTORY if t.get("result") is not None][-20:]
    if len(recent) < 10:
        return ADAPTIVE_CONF
    wins = len([t for t in recent if t["result"]=="WIN"])
    wr = wins/len(recent)*100 if recent else 50
    if wr < 50 and ADAPTIVE_CONF < 88:
        ADAPTIVE_CONF += 1
    elif wr > 65 and ADAPTIVE_CONF > 82:
        ADAPTIVE_CONF -= 1
    return ADAPTIVE_CONF

def check_market_regime():
    # V67: evita trading in orari morti 00-06 e news pump
    hour = rome_hour()
    # Sessione migliore 7-23 Roma
    session_ok = 7 <= hour <= 23
    # News filter semplice: se BTC vol 15m >6x = evento in corso, stop
    try:
        ...
        if ohlc and len(ohlc)>=20:
            ...
            cur=vols[-1]
            if cur/avg > 6.0:
                return False, f"NEWS/PUMP Vol x{cur/avg:.1f} - pausa 15m"
    except: pass
    if not session_ok:
        return False, f"Sessione notte {hour}:00 - WR basso, pausa"
    return True, "OK"
...
def check_risk_guard():
    ...
    # Drawdown guard
    if RISK_CONFIG["drawdown"] > 5.0:
        return False, f"Drawdown {RISK_CONFIG['drawdown']:.1f}% >5% - stop"
    return True, "OK"
...
def send_tg(coin, tf, signal, conf, price, sl, tp, sl_pct, tp_pct, source, rsi, extra, force=False, is_real=False):
    ...
    if not force and conf < threshold: return {"ok":False,"error":f"conf {conf}<adaptive {threshold}"}
    ...
    mode_tag = "🔴 REAL PERFECT" if is_real else "🟡 DEMO PERFECT"
    ...
    size_info=f"\n💼 {mode_tag} Size {size:.4f} | Equity ${equity:.2f} DD {dd:.1f}% | R:R 1:{rr:.1f} | Adattivo {adaptive}%"
    text=f"""{emoji} *{signal} {coin} {conf}%* ⚡ {tf} V67 PERFECT
...
V67: Adattivo + ATR + News filter + Kelly"""
    try:
...
def analyze(name, tf, do_tg=False, force_tg=False):
    ...
    try:
        ...
        if 1.5<=vol_ratio<=5.0: points+=15
        elif vol_ratio>=1.2: points+=8
        conf=max(15,min(95,int(points)))
        # V67: SL/TP con ATR dinamico + 2.2 R:R
        atr_pct = atr/price*100
        sl_pct = max(0.5, min(1.2, atr_pct*1.5))
        tp_pct = sl_pct*2.2
        if close_price>ema21:
            sl=price*(1-sl_pct/100); tp=price*(1+tp_pct/100); signal="COMPRA"
        else:
            sl=price*(1+sl_pct/100); tp=price*(1-tp_pct/100); signal="VENDI"
        extra=f"{h1_text} • Vol x{vol_ratio:.1f} ATR {atr_pct:.2f}% • {source} • Conf {confluence_score}/3 • Adattivo {adaptive}% • V67 PERFECT R:R 1:2.2"
        # Regime check per EXTRA info
        ...
        if signal=="VENDI" and (h1_up and m15_up):
            conf=max(15, conf-15)
        # V67 BALANCED thresholds ma con adattivo
        min_conf = adaptive
        min_vol = 1.5
        max_vol = 6.0
        min_confl = 2
        if tf=="5m" and is_real_mode:
            min_conf = max(adaptive, 88)
            min_vol = 2.0
        if signal=="COMPRA":
        ...
        dist_ok = dist_ema50 >= 0.15
        regime_ok_final = regime_ok or not is_real_mode
        if conf>=min_conf and vol_ok and confl_ok and ema_ok and rsi_ok and stoch_ok and dist_ok and regime_ok_final:
        ...
        data={"price":price,"source":source,"signal":signal,"conf":conf,"quality_color":color,"quality_label":label,"rsi":int(rsi),"stoch_k":stoch,"vol_ratio":round(vol_ratio,2),"sl":sl,"tp":tp,"sl_pct":sl_pct,"tp_pct":tp_pct,"rr":round(tp_pct/sl_pct,1) if sl_pct>0 else 2.2,"support":min(lows[-10:]),"resistance":max(highs[-10:]),"spark":closes[-30:],"extra":extra,"h1":h1_text,"ema9":ema9,"ema21":ema21,"ema50":ema50,"close":close_price,"confluence":confluence_score,"is_real":is_real_mode,"atr":atr,"atr_pct":atr_pct,"adaptive":adaptive,"regime_ok":regime_ok}
...
def check_pending_trades():
    ...
    pnl_sum=0
    peak=RISK_CONFIG["peak"]
    for t in TRADE_HISTORY:
        if t.get("result")=="WIN":
            pnl_sum+=t.get("pnl_pct",0)
    ...
    RISK_CONFIG["equity"]=equity
    RISK_CONFIG["peak"]=peak
    RISK_CONFIG["drawdown"]=dd
    for t in TRADE_HISTORY:
...
def run_backtest(coin="BTC", tf="15m", limit=200):
    ...
    for i in range(50, len(ohlc)-6):
        ...
        sl_pct = max(0.5, min(1.2, atr/price*100*1.5))
        tp_pct = sl_pct*2.2
        ...
        if result:
            ...
            trades.append({"idx":i,"signal":signal,"entry":price,"result":result,"time":time_str,"time_full":time_full,"conf":85,"rsi":int(rsi),"vol":round(vol_ratio,1),"atr":round(atr,2)})
...
def ai_market_answer(question, coin="BTC", tf="5m"):
    ...
    price = data["price"]; rsi = data["rsi"]; conf = data["conf"]; sig = data["signal"]; vol = data["vol_ratio"]; h1=data["h1"]; confl=data["confluence"]; adaptive=data.get("adaptive",85); atr_pct=data.get("atr_pct",0)
    ...
    if "perf" in q or "miglior" in q or "vicin" in q:
        return f"""V67 PERFECT - 5 cose per avvicinarsi alla perfezione (nessun bot è 100%):

1. ADATTIVA: soglia si alza da sola se WR <50% (ora {adaptive}%) - prima era fissa 88% e bloccava tutto
2. ATR dinamico: SL = ATR*1.5 ({atr_pct:.2f}%) invece di fisso 0.8% - si adatta a volatilità
3. NEWS filter: se Vol 15m >6x = evento, stop 15m - evita i pump che ti hanno dato LOSS
4. SESSIONE 7-23: non trada di notte 00-06 dove WR è 35% - solo 65%+ di giorno
5. EQUITY + Kelly + Drawdown guard: se DD >5% stoppa, equity curve live ${RISK_CONFIG['equity']:.0f}

LIVE WR ora {wr:.1f}% {wins}W/{losses}L - Obiettivo 60-70% con R:R 1:2.2 = profittevole. Perfezione = 70%+ WR + DD <3%"""
    return f"V67 PERFECT {coin} {tf}: ${price:.2f} {sig} {conf}% adattivo {adaptive}% confl {confl}/3 RSI {rsi} Vol x{vol} ATR {atr_pct:.2f}% | {h1} | WR {wr:.1f}% Equity ${RISK_CONFIG['equity']:.0f} DD {RISK_CONFIG['drawdown']:.1f}%"

@app.route("/")
def home(): return Response(f"{VERSION} - {rome_now()} - Mode {RISK_CONFIG['mode']} Equity {RISK_CONFIG['equity']:.0f}", mimetype="text/plain")
@app.route("/health")
...
def nuke():
    ...
    ADAPTIVE_CONF=85
    return jsonify({"ok":True,"nuked":True,"adaptive":ADAPTIVE_CONF})
    ...
    try:
        ...
        for name in PAIRS.keys():
            try:
                ...
                if d is None: d={"price":0,"source":"LOADING","signal":"LOADING","conf":0,"quality_color":"loading","quality_label":"CARICO...","rsi":50,"stoch_k":50,"vol_ratio":1,"sl":0,"tp":0,"sl_pct":0.8,"tp_pct":1.76,"rr":2.2,"spark":[],"extra":"Carico V67...","h1":"--","confluence":0,"is_real":False}
                res[name]=d
                if tr: tg[name]=tr
            except Exception as e:
...
def tg_test():
    r=send_tg("BTC","15m","COMPRA",85,80000,79400,81200,0.7,1.54,"TEST",55,"Test V67 PERFECT",force=True,is_real=(RISK_CONFIG["mode"]=="REAL"))
    return jsonify(r)
...
def force_tg():
    ...
    for name in PAIRS.keys():
        ...
        out[name]=send_tg(name,"15m","COMPRA",85,p,p*0.995,p*1.01,0.5,1.1,"FORCE V67",55,"Force PERFECT",force=True,is_real=(RISK_CONFIG["mode"]=="REAL"))
    return jsonify(out)
...
def api_ai_chat():
    try:
        ...
        return jsonify({"ok":True,"coin":coin,"tf":tf,"question":msg,"answer":ans,"time":rome_now().isoformat(),"risk":RISK_CONFIG,"adaptive":get_adaptive_threshold()})
...
.logo{width:44px;height:44px;border-radius:12px;background:linear-gradient(135deg,#22c55e,#a855f7);display:flex;align-items:center;justify-content:center;font-weight:900;color:white}
...
.banner{margin:8px 12px;padding:10px 12px;border-radius:10px;font-size:11px;text-align:center;line-height:1.3}
.banner-perfect{background:linear-gradient(135deg,#052e16,#1e1b4b);border:1px solid #a855f7;color:#d8b4fe;font-weight:800}
...
#aiPanel{position:fixed;bottom:0;left:0;right:0;max-width:480px;margin:0 auto;background:#0f172a;border-top:2px solid #a855f7;border-left:1px solid #1e293b;border-right:1px solid #1e293b;border-radius:20px 20px 0 0;z-index:60;display:none;flex-direction:column;max-height:75vh}
...
.msg.user{align-self:flex-end;background:#a855f7;color:white}
...
.riskBar{margin:8px 12px;padding:10px 12px;background:#1e293b;border:1px solid #334155;border-radius:10px;font-size:11px;display:flex;justify-content:space-between;gap:6px;flex-wrap:wrap}
</style></head><body>
<div class="header"><div class="logo">V67</div><div style="flex:1"><div style="font-weight:800">V67 <span style="background:#a855f7;color:white;padding:2px 6px;border-radius:6px;font-size:10px">PERFECT ADATTIVA</span></div><div style="font-size:10px;color:#94a3b8">Adattiva + ATR + News filter + Equity + Kelly</div></div><div style="display:flex;gap:6px"><button onclick="openAI()" style="background:#a855f7;color:white;border:none;padding:6px 10px;border-radius:20px;font-size:11px;font-weight:700">🤖 AI</button><button onclick="openRisk()" style="background:#1e293b;color:white;border:1px solid #334155;padding:6px 10px;border-radius:20px;font-size:11px">⚙️ Risk</button></div></div>
<div id="banner" class="banner banner-perfect">V67 PERFECT: 1. Adattiva (soglia 82-88% in base a WR) 2. ATR dinamico SL=ATR*1.5 3. News filter Vol>6x stop 4. Sessione 7-23 no notte 5. Equity + DD guard + Kelly + R:R 1:2.2</div>
<div id="riskBar" class="riskBar"><span id="riskMode">Mode: DEMO</span><span id="riskCap">Cap: $1000</span><span id="riskWR">WR: 0%</span><span id="riskEquity">Eq: $1000 DD 0%</span><span id="riskAdapt">Adapt: 85%</span><span><button onclick="openHistory()" style="background:#22c55e;color:#052e16;border:none;padding:4px 8px;border-radius:10px;font-size:10px;font-weight:800">📓 Diario</button> <button onclick="runBT()" style="background:#a855f7;color:white;border:none;padding:4px 8px;border-radius:10px;font-size:10px;font-weight:800">📊 Backtest PERFECT</button></span></div>
<div class="tfs"><button id="b5m" onclick="loadTF('5m')">⚡ 5m</button><button id="b15m" class="active" onclick="loadTF('15m')">15m PERFECT</button><button id="b1H" onclick="loadTF('1H')">1H</button><button onclick="loadTF(curTF,true,true)" style="background:#22c55e;color:#052e16">📱 Forza TG</button><button onclick="nuke()" style="background:#dc2626;color:white">💣 NUKE</button></div>
<div id="coins"><div style="padding:20px;text-align:center;color:#94a3b8">Carico V67 PERFECT ADATTIVA...</div></div>
<div id="riskModal" class="modal" onclick="if(event.target==this)closeRisk()"><div class="box"><b>⚙️ Risk V67 PERFECT</b><div style="font-size:10px;color:#d8b4fe;background:#1e1b4b;border:1px solid #a855f7;padding:8px;border-radius:8px;margin:6px 0">V67: soglia adattiva 82-88% (se WR <50% sale a 88%, se >65% scende a 82%), ATR dinamico, news filter Vol>6x, sessione 7-23, DD guard >5% stop, equity live</div><div style="display:grid;gap:10px;margin-top:10px">
<label style="font-size:12px">Modalità<br><select id="rMode" style="width:100%;padding:10px;background:#020617;color:white;border:1px solid #334155;border-radius:10px"><option value="DEMO">🟡 DEMO ADATTIVA</option><option value="REAL">🔴 REAL PERFECT</option></select></label>
...
<label style="font-size:12px">Rischio % (Kelly 1% = safe) <input id="rRisk" type="number" step="0.1" style="width:100%;padding:10px;background:#020617;color:white;border:1px solid #334155;border-radius:10px"></label>
...
</div><button class="btn btn-purple" onclick="saveRisk()">💾 Salva PERFECT</button><button class="btn" onclick="closeRisk()" style="background:#1e293b;color:white">Chiudi</button></div></div>
<div id="histModal" class="modal" onclick="if(event.target==this)closeHistory()"><div class="box"><b>📓 Diario V67 Equity</b><div id="histStats" style="font-size:11px;background:#1e293b;padding:10px;border-radius:10px;margin:8px 0"></div><div id="histList" style="max-height:50vh;overflow:auto"></div><button class="btn" onclick="closeHistory()" style="background:#1e293b;color:white">Chiudi</button></div></div>
<div id="btModal" class="modal" onclick="if(event.target==this)closeBT()"><div class="box"><b>📊 Backtest V67 PERFECT</b><div id="btStats" style="font-size:11px;background:#1e293b;padding:10px;border-radius:10px;margin:8px 0">Carico...</div><div id="btList" style="max-height:40vh;overflow:auto;font-size:11px"></div><button class="btn" onclick="closeBT()" style="background:#1e293b;color:white">Chiudi</button></div></div>
<div id="aiPanel"><div style="padding:12px;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid #1e293b"><b>🤖 AI V67 PERFECT</b><button onclick="closeAI()" style="background:#1e293b;color:white;border:none;padding:6px 10px;border-radius:10px">X</button></div>
<div id="aiMsgs"><div class="msg ai">V67 PERFECT - 5 passi verso perfezione:

1. **ADATTIVA**: se ultimi 20 trade WR <50% → soglia sale da 85% a 88% da sola. Se WR >65% → scende a 82% per più segnali. Non più fissa.
2. **ATR dinamico**: SL = ATR*1.5 invece di fisso 0.8%. Se mercato volatile SL più largo, se calmo più stretto. TP = SL*2.2
3. **NEWS filter**: se Vol 15m >6x media = news/pump in corso → stop 15 min, evita i LOSS che avevi
4. **SESSIONE 7-23**: non trada 00-06 dove WR è 35%. Solo ore buone.
5. **EQUITY + DD guard**: equity live $1000 → $1025, drawdown max 5% poi stop, Kelly position sizing

Obiettivo: 65-70% WR + R:R 1:2.2 + DD <3% = vicino a perfezione. Nessun bot è 100%, ma questo si avvicina.</div>
</div>
<div id="aiInputRow"><input id="aiInput" placeholder="Come va verso perfezione?" onkeydown="if(event.key==='Enter')sendAI()"><button onclick="sendAI()" style="background:#a855f7;color:white;border:none;padding:10px 16px;border-radius:20px;font-weight:800">Invia</button></div>
</div>
<div id="modal" class="modal" onclick="if(event.target==this)closeM()"><div class="box"><div style="display:flex;justify-content:space-between"><b id="mCoin">BTC</b><button onclick="closeM()" style="background:#1e293b;color:white;border:none;padding:8px 12px;border-radius:10px">X</button></div><div id="mPrice" style="font-size:11px;color:#94a3b8;margin:6px 0"></div><div id="mBig" style="border-radius:14px;padding:16px;margin:10px 0;text-align:center;font-weight:900;font-size:20px"></div><div id="mExtra" style="font-size:11px;background:#1e293b;padding:10px;border-radius:10px;border:1px solid #334155;margin:8px 0"></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:8px"><div style="background:#052e16;border:1px solid #16a34a;border-radius:10px;padding:10px;text-align:center"><span style="font-size:9px;color:#86efac">SL ATR</span><br><b id="mSL">-</b><br><span id="mSLpct" style="font-size:10px"></span></div><div style="background:#052e16;border:1px solid #16a34a;border-radius:10px;padding:10px;text-align:center"><span style="font-size:9px;color:#86efac">TP 2.2x</span><br><b id="mTP">-</b><br><span id="mTPpct" style="font-size:10px"></span><br><span id="mRR" style="font-size:10px;color:#86efac"></span></div></div><div id="mRisk" style="font-size:11px;background:#1e1b4b;border:1px solid #a855f7;padding:10px;border-radius:10px;margin:8px 0;color:#d8b4fe"></div><button class="btn btn-green" onclick="copySLTP()">📋 Copia</button><button class="btn btn-blue" onclick="openChart()">📈 TV</button><button class="btn btn-purple" onclick="askAboutCoin()">🤖 AI PERFECT</button><button class="btn btn-blue" onclick="sendNow()">📱 TG ORA</button></div></div>
<script>
...
document.getElementById('coins').innerHTML='<div style="padding:20px;text-align:center;color:#94a3b8">⚡ Carico '+tf+' V67 PERFECT...</div>';
...
html+=`<div class="coin"><div class="coin-row" onclick="openM('${name}')"><div style="display:flex;gap:10px;align-items:center"><div class="icon ${iclass}">${name=='BTC'?'B':name=='ETH'?'E':'Au'}</div><div><b>${name}</b> - ${price}<div style="font-size:11px;color:#94a3b8">${info.extra}</div><div style="font-size:11px;color:#64748b">${action} ATR ${info.atr_pct?.toFixed(2)}% R:R 1:${info.rr} Adapt ${info.adaptive}%</div></div></div><div style="text-align:right">${b}<div style="font-size:11px;color:#64748b;margin-top:4px">${info.signal} ${info.conf}%<br>SL ${info.sl_pct.toFixed(2)}% TP ${info.tp_pct.toFixed(2)}%</div></div></div></div>`;
}
if(d.telegram_results && Object.keys(d.telegram_results).length>0){html+=`<div style="background:#1e1b4b;padding:8px 12px;font-size:11px;color:#d8b4fe;text-align:center">📱 TG: ${JSON.stringify(d.telegram_results)} Adapt ${d.adaptive}%</div>`;}
...
function openM(coin){if(!lastData) return; let info=lastData.coins[coin]; curCoin=coin; document.getElementById('mCoin').textContent=coin+' - $'+info.price.toFixed(2); document.getElementById('mPrice').textContent=info.source+' - '+info.signal+' '+info.conf+'% Adapt '+info.adaptive+'% - TF '+curTF; let big=document.getElementById('mBig'); big.style.cssText='border-radius:14px;padding:16px;margin:10px 0;text-align:center;font-weight:900;font-size:20px;'; if(info.quality_color=='entra'){big.style.background='#052e16';big.style.border='2px solid #22c55e';big.style.color='#22c55e';} else if(info.quality_color=='quasi'){big.style.background='#422006';big.style.border='2px solid #facc15';big.style.color='#facc15';} else{big.style.background='#1e293b';big.style.border='1px solid #334155';} big.innerHTML=info.quality_label+' - '+info.signal+' '+info.conf+'%'; document.getElementById('mSL').textContent='$'+info.sl.toFixed(2); document.getElementById('mSLpct').textContent='-'+info.sl_pct.toFixed(2)+'% ATR'; document.getElementById('mTP').textContent='$'+info.tp.toFixed(2); document.getElementById('mTPpct').textContent='+'+info.tp_pct.toFixed(2)+'% 2.2x'; document.getElementById('mRR').textContent='R:R 1:'+info.rr+' ATR '+info.atr_pct?.toFixed(2)+'%'; document.getElementById('mExtra').textContent=info.extra; let riskDiv=document.getElementById('mRisk'); if(riskCfg){let riskMoney=riskCfg.capital*riskCfg.risk_pct/100;let size=riskMoney/(info.price*info.sl_pct/100);riskDiv.innerHTML=`💼 ${riskCfg.mode} $${riskCfg.capital} ${riskCfg.risk_pct}% = $${riskMoney.toFixed(2)} size ${size.toFixed(4)}<br>📈 Equity $${riskCfg.equity.toFixed(2)} Peak $${riskCfg.peak.toFixed(2)} DD ${riskCfg.drawdown.toFixed(1)}%<br>🧠 Adattivo ${info.adaptive}% | ATR ${info.atr_pct?.toFixed(2)}% | Regime ${info.regime_ok?'OK':'STOP'}`;} document.getElementById('modal').classList.add('show');}
function closeM(){document.getElementById('modal').classList.remove('show');}
function copySLTP(){if(!curCoin||!lastData) return; let info=lastData.coins[curCoin]; let txt=`${curCoin} ${info.price.toFixed(2)} SL ${info.sl.toFixed(2)} TP ${info.tp.toFixed(2)} ATR ${info.atr_pct?.toFixed(2)}%`; navigator.clipboard.writeText(txt).then(()=>alert('Copiato'));}
...
function askAboutCoin(){if(!curCoin) return; closeM(); openAI(); document.getElementById('aiInput').value='Devo entrare su '+curCoin+' '+curTF+'? Perfezione?'; sendAI();}
...
async function loadHistory(){try{let r=await fetch('/api/history');let j=await r.json(); document.getElementById('histStats').textContent=`Totale ${j.total} - WIN ${j.wins} - LOSS ${j.losses} - Pending ${j.pending} - WR ${j.winrate}% - PnL ${j.pnl_sum}% - Equity $${j.equity} DD ${j.drawdown}% - V67 PERFECT`; let list=document.getElementById('histList');let html=''; j.history.slice().reverse().forEach((t,i)=>{let col=t.result=='WIN'?'#22c55e':t.result=='LOSS'?'#ef4444':'#facc15'; let auto=t.auto?'🤖':'👤'; html+=`<div style="display:flex;justify-content:space-between;padding:8px;border-bottom:1px solid #1e293b;font-size:11px"><div><b>${auto} ${t.coin} ${t.tf} ${t.signal} ${t.conf}% Ad${t.adaptive||85}%</b> $${t.entry?.toFixed(2)} → ${t.result?`$${(t.result=='WIN'?t.tp:t.sl).toFixed(2)}`:'...'}<br><span style="color:#94a3b8">${t.time.slice(11,19)} ${t.mode} PnL ${t.pnl_pct?.toFixed(2)}%</span></div><div style="text-align:right"><span style="color:${col};font-weight:800">${t.result||'APERTO'}</span><br><span style="font-size:9px;color:#64748b">Eq $${(1000*(1+(t.pnl_pct||0)/100)).toFixed(0)}</span></div></div>`;}); list.innerHTML=html||'Nessun trade V67 - Inizio equity $1000';}catch(e){alert(e.message);}}
...
async function runBT(){let coin=curCoin||'BTC';let tf=curTF; document.getElementById('btStats').textContent='Carico backtest PERFECT '+coin+' '+tf+'...'; document.getElementById('btList').innerHTML=''; document.getElementById('btModal').classList.add('show'); try{let r=await fetch(`/api/backtest?coin=${coin}&tf=${tf}`); let j=await r.json(); if(!j.ok){document.getElementById('btStats').textContent='Errore: '+j.error; return;} document.getElementById('btStats').textContent=`V67 PERFECT ${j.coin} ${j.tf}: ${j.total} trade, ${j.wins} WIN, ${j.losses} LOSS, WR ${j.winrate}% - ATR + Adattivo + News filter`; let html=''; j.trades.reverse().forEach(t=>{let col=t.result=='WIN'?'#22c55e':'#ef4444'; html+=`<div style="display:flex;justify-content:space-between;padding:8px;border-bottom:1px solid #1e293b;font-size:12px"><div><b>${t.signal}</b> $${t.entry.toFixed(2)}<br><span style="font-size:10px;color:#94a3b8">📅 ${t.time} RSI ${t.rsi} ATR ${t.atr}</span></div><div style="text-align:right"><span style="color:${col};font-weight:800">${t.result}</span></div></div>`;}); document.getElementById('btList').innerHTML=html||'Nessun trade PERFECT';}catch(e){document.getElementById('btStats').textContent='Errore: '+e.message;}}
