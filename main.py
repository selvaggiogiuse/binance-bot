...
VERSION = "V54 - FIX KRAKEN FALLBACK OHLC - 23/08/2026"
...
def get_live_price_ticker(name):
    ...
    try:
        ...
        if r.status_code==200:
            return float(r.json()['price']), f"BINANCE:{symbol}", "binance"
    ...
    try:
        ...
        if r.status_code==200:
            ...
            if result:
                ...
                return float(result[first_key]["c"][0]), f"KRAKEN:{kp}", "kraken"
    except:
        pass
...
def fetch_binance_klines(symbol, interval, limit=200):
    try:
        ...
        if r.status_code!=200:
            return [], f"binance_{r.status_code}"
        data=r.json()
        ohlc=[]
        for k in data:
            ohlc.append({"time":int(k[0]/1000),"open":float(k[1]),"high":float(k[2]),"low":float(k[3]),"close":float(k[4]),"volume":float(k[5])})
        return ohlc, f"binance_ok_{len(ohlc)}"
    except Exception as e:
        return [], f"binance_exc_{str(e)[:20]}"

def fetch_kraken_ohlc(name, interval, limit=200):
    try:
        kraken_map={"BTC":"XXBTZUSD","ETH":"XETHZUSD","ORO":"PAXGUSD"}
        kp=kraken_map.get(name,"XXBTZUSD")
        kraken_interval_map={"5m":1,"15m":15,"1h":60,"4h":240,"1d":1440,"1d":1440}
        ...
        if r.status_code!=200:
            return [], f"kraken_{r.status_code}"
        j=r.json(); result=j.get("result",{})
        if not result:
            return [], "kraken_empty"
        ...
        return ohlc, f"kraken_ok_{len(ohlc)}"
    except Exception as e:
        return [], f"kraken_exc_{str(e)[:20]}"

def fetch_ohlc_with_fallback(name, interval, limit=200):
    symbol=PAIRS_OHLC.get(name,"BTCUSDT")
    ohlc, status = fetch_binance_klines(symbol, interval, limit)
    if ohlc and len(ohlc)>=20:
        return ohlc, status
    # Fallback Kraken
    ohlc2, status2 = fetch_kraken_ohlc(name, interval, limit)
    if ohlc2 and len(ohlc2)>=20:
        return ohlc2, status2
    return [], f"{status}+{status2}"

def analyze_coin(name, tf):
    interval=TF_MAP.get(tf,"1h")
    live_price, source, live_status = get_live_price_ticker(name)
    ohlc, ohlc_status = fetch_ohlc_with_fallback(name, interval, 200)
    
    if not ohlc or len(ohlc)<20:
        if live_price is None:
            return None
        # Debug: mostra perché le candele falliscono
        return {"price":live_price,"real_price":live_price,"close_price":live_price,"source":source,"ohlc_status":ohlc_status,"signal":"ASPETTA","conf":52,"quality_color":"wait","quality_label":"ASPETTA","quality_score":45,"quality_simple":f"DEBUG FALLBACK: prezzo da {source} ok, ma candele fallite -> {ohlc_status}. TF {tf} {interval} - Binance bloccato su Render, uso Kraken ma ha fallito. Riprovo tra poco.","rsi":50,"ema50":live_price,"ema200":live_price,"st_trend":0,"st_val":live_price,"stoch_k":50,"vwap":live_price,"support":live_price*0.98,"resistance":live_price*1.02,"adx":20,"vol_ratio":1.0,"sl":live_price*0.97,"tp":live_price*1.03}
    ...
    src = source if live_price is not None else "CANDLE"
    ...
    if conf>=65 and st_trend==1 and close_price>ema50:
        ...
        quality_simple=f"[{src}+{ohlc_status}] LIVE ${price:.2f} - RSI{int(rsi)} EMA{int(ema50)}/{int(ema200)} ST{'UP' if st_trend==1 else 'DOWN'} Stoch{stoch_k} VWAP{int(vwap)} Sup{int(support)} Res{int(resistance)} ADX{adx} Volx{vol_ratio} = {points}/{max_points} -> {conf}% - TF {tf}"
        ...
        quality_simple=f"[{src}+{ohlc_status}] ribasso LIVE ${price:.2f} RSI{int(rsi)} Stoch{stoch_k} ADX{adx} = {conf}% - TF {tf}"
    else:
        if conf>=65:
            signal="COMPRA"; quality_color="quasi"; quality_label="QUASI PRONTO"; quality_simple=f"[{src}+{ohlc_status}] {points}/{max_points} ({conf}%) quasi pronto TF {tf} LIVE ${price:.2f}"
        else:
            signal="ASPETTA"; quality_color="wait"; quality_label="ASPETTA"; quality_simple=f"[{src}+{ohlc_status}] LIVE ${price:.2f} - {conf}% TF {tf} - RSI{int(rsi)} EMA{int(ema50)} ST{'UP' if st_trend==1 else 'DOWN'}"
    
    sl=price*0.98 if signal=="COMPRA" else price*1.02
    tp=price*1.04 if signal=="COMPRA" else price*0.96
    return {"price":price,"real_price":price,"close_price":close_price,"source":src,"ohlc_status":ohlc_status,"signal":signal,"conf":int(conf),"quality_color":quality_color,"quality_label":quality_label,"quality_score":int(conf),"quality_simple":quality_simple,"rsi":int(rsi),"ema50":ema50,"ema200":ema200,"st_trend":st_trend,"st_val":st_val,"stoch_k":stoch_k,"vwap":vwap,"support":support,"resistance":resistance,"adx":adx,"vol_ratio":vol_ratio,"sl":sl,"tp":tp}
...
def api_signals():
    ...
    for name in PAIRS_LIVE.keys():
        ...
        if data is None:
            data={"price":80000,"real_price":80000,"close_price":80000,"source":"FALLBACK","ohlc_status":"both_failed","signal":"ASPETTA","conf":52,"quality_color":"wait","quality_label":"ASPETTA","quality_score":45,"quality_simple":"Entrambe le fonti fallite","rsi":50,"ema50":0,"ema200":0,"st_trend":0,"st_val":0,"stoch_k":50,"vwap":0,"support":0,"resistance":0,"adx":20,"vol_ratio":1.0,"sl":0,"tp":0}
        result[name]=data
        debug_info[name]=data.get("ohlc_status","?")
    return jsonify({"ok":True,"tf":tf,"coins":result,"debug":debug_info,"time":rome_now().isoformat(),"version":VERSION})
...
<div>
<div style="font-weight:800;font-size:16px">VENDI - <span style="background:#22c55e;color:#052e16;padding:2px 6px;border-radius:6px;font-size:12px">V54</span> FIX TIMEFRAME</div>
<div style="font-size:11px;opacity:0.8">Fix Binance 451 + Kraken fallback - Ogni TF diverso</div>
</div>
</div>
<div class="tfs">
...
<div id="debug" style="font-size:10px;color:#94a3b8;padding:4px 16px"></div>
...
async function loadTF(tf){
  ...
  document.getElementById('coins').innerHTML='<div style="padding:20px;text-align:center">Carico V54 '+tf+' con fallback Kraken...</div>';
  try{
    var res=await fetch('/api/signals?tf='+tf);
    var d=await res.json();
    lastData=d;
    document.getElementById('debug').textContent='Debug TF='+tf+' | '+JSON.stringify(d.debug)+' | '+d.time;
    var html='';
    for(var name in d.coins){
      ...
      html+='<div class="coin-row" onclick="openDetails(\\''+name+'\\')"><div style="display:flex;gap:10px;align-items:center"><div class="coin-icon '+iconClass+'">'+ico+'</div><div><b style="font-size:16px">'+name+'</b> - '+price+'<span style="font-size:8px;color:#22c55e;margin-left:4px">'+info.source+' '+info.ohlc_status+'</span><div style="font-size:12px;color:#64748b;margin-top:2px">'+actionText+'</div></div></div><div style="text-align:right">'+qBadge+'<div style="font-size:11px;color:#64748b;margin-top:4px">'+info.signal+' '+info.conf+'%</div></div></div>';
    }
...
async function openDetails(coin){
  ...
  document.getElementById('mPrice').textContent=info.source+' '+info.ohlc_status+' - '+info.signal+' '+info.conf+'% - TF '+curTF+' - LIVE $'+info.real_price.toFixed(2)+' (close $'+info.close_price.toFixed(2)+')';
  ...
  if(info.quality_color=='entra'){
    big.innerHTML='<div style="font-size:24px;font-weight:900;color:'+(info.signal=='COMPRA'?'#166534':'#991b1b')+'">'+info.quality_label+' - '+info.signal+'</div><div style="font-size:13px;color:#475569;margin-top:6px">Score '+info.quality_score+'% - SL $'+info.sl.toFixed(0)+' TP $'+info.tp.toFixed(0)+'</div>';
  ...
  document.getElementById('mExpert').textContent='Fonte: '+info.source+' '+info.ohlc_status+' - TF '+curTF;
  document.getElementById('modal').classList.add('show');
}
function closeModal(){ document.getElementById('modal').classList.remove('show'); }

loadTF('1H');
setInterval(function(){loadTF(curTF);}, 20000);
</script>
</body></html>
"""
