...
VERSION = "V60 LOOSE - MAX TRADE - NO FILTRI STRETTI + FIX FUTURE"
COOLDOWN = 300
...
def send_tg(coin, tf, signal, conf, price, sl, tp, sl_pct, tp_pct, source, rsi, extra, force=False):
    ...
    text=f"""{emoji} *{signal} {coin} {conf}%* ⚡ {tf} V60 LOOSE
...
def analyze(name, tf, do_tg=False, force_tg=False):
    ...
    # V60 LOOSE: NESSUN FILTRO ADX/RSI - massimo numero di trade
    ...
    extra=f"{h1_text} • Vol x{vol_ratio:.1f} • {source} • LOOSE MODE"
...
@app.route("/")
def home(): return Response(f"{VERSION} - {rome_now()} - LOOSE MAX TRADE", mimetype="text/plain")
@app.route("/health")
...
def tg_test():
    r=send_tg("BTC","5m","COMPRA",85,80000,79400,81200,0.7,1.5,"TEST",58,"Test V60 LOOSE",force=True)
    return jsonify(r)
...
def force_tg():
    ...
    for name in PAIRS.keys():
        ...
        out[name]=send_tg(name,"15m","COMPRA",93,p,p*0.995,p*1.01,0.5,0.9,"FORCE LOOSE",53,"Force LOOSE",force=True)
    return jsonify(out)
...
<div class="header"><div class="logo">V60</div><div style="flex:1"><div style="font-weight:800">VENDI V60 <span style="background:#facc15;color:#422006;padding:2px 6px;border-radius:6px;font-size:10px">LOOSE MAX TRADE</span></div><div style="font-size:10px;color:#94a3b8">Zero filtri stretti • Max occasioni • Fix futuro</div></div><div><button onclick="testTG()" style="background:#0088cc;color:white;border:none;padding:6px 10px;border-radius:20px;font-size:11px;font-weight:700">📱 Test</button></div></div>
<div id="banner" class="banner b-off">Verifico V60 LOOSE...</div>
<div class="tfs"><button id="b5m" onclick="loadTF('5m')">⚡ 5m LOOSE</button><button id="b15m" class="active" onclick="loadTF('15m')">15m LOOSE</button><button id="b1H" onclick="loadTF('1H')">1H LOOSE</button><button onclick="loadTF(curTF,true,true)" style="background:#22c55e;color:#052e16">📱 Forza TG</button><button onclick="clearTG()" style="background:#dc2626;color:white">🗑️ Clear</button></div>
<div id="coins"><div style="padding:20px;text-align:center;color:#94a3b8">Carico V60 LOOSE - max trade...</div></div>
...
async function checkTG(){try{let r=await fetch('/api/telegram_config');let j=await r.json();let b=document.getElementById('banner');if(j.enabled){let warn=j.future_keys&&j.future_keys.length>0?' ⚠️ BLOCCO: '+j.future_keys.join(',')+' - Clear!':'';b.className='banner '+(warn?'b-off':'b-on');b.innerHTML='✅ V60 LOOSE ON - Soglia 75% - MAX TRADE - Nessun filtro ADX - Ultimo: '+JSON.stringify(j.last)+warn;}else{b.className='banner b-off';b.innerHTML='❌ Telegram OFF';}}catch{}}
async function clearTG(){if(!confirm('Cancello cooldown?'))return;try{let r=await fetch('/api/clear_telegram');alert('✅ Cancellato!');checkTG();loadTF(curTF);}catch(e){alert(e.message);}}
...
document.getElementById('coins').innerHTML='<div style="padding:20px;text-align:center;color:#94a3b8">⚡ Carico '+tf+' LOOSE...'+(withTG?' + TG':'')+'</div>';
try{
...
async function testTG(){try{let r=await fetch('/api/telegram_test'); let j=await r.json(); alert(j.ok?'✅ Test V60 LOOSE!':'❌ '+j.error);}catch(e){alert(e.message);}}
