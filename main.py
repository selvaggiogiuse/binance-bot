"""
VAPID FIX - chiave valida generata ora
PUBLIC: BHWs4iOkU3pKk6E46BXj3iL6jopscCgpcQcH6i8xDCYhbFUAT8pwvGxMGhl3v9T7TChtOVpaAF48t8cWFaWtimQ
PRIVATE: wA-4RFSsnHB2oSSYQ_tELw9Mo6ljDaqpKVSnQH9EpF0
Metti queste 2 in Render -> Environment
"""
...
def checker():
    print("Checker VAPID FIX avviato")
    while True:
        try:
            for tf in ["1H","4H"]:
...
@app.route("/api/ping")
def ping(): return jsonify({"ok":True,"history":len(history),"subs":len(subscriptions),"vapid_pub":VAPID_PUBLIC_KEY[:20]+"..."})
...
def sub():
    ...
    if s and s not in subscriptions:
        ...
        print(f"Nuova subscription salvata totale {len(subscriptions)}")
    return jsonify({"ok":True,"total":len(subscriptions)})
@app.route("/api/push/test", methods=["POST"])
def testp():
    d=request.get_json(silent=True) or {}
    sent=send_push(f"TEST {d.get('coin','BTC')} COMPRA 75%", f"Test VAPID FIX - {datetime.now().strftime('%H:%M:%S')}", coin=d.get('coin','BTC'), tf=d.get('tf','4H'))
...
async function subscribePush(){
  try{
    ...
    if(perm!=='granted'){alert('Permesso notifiche negato. Attivalo in impostazioni Chrome');return;}
    ...
    document.getElementById('subStatus').innerText='Push: ATTIVO [SERVER] '+j.total;
    alert('Push attiva! Totale iscritti: '+j.total);
  }catch(e){alert('Errore push: '+e.message); console.error(e);}
}
async function testPush(){try{const r=await fetch('/api/push/test',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({coin:'BTC',tf:curTF})});const j=await r.json();alert('Test inviato a '+j.sent_to+' dispositivi. Chiudi app e aspetta 5 sec');}catch(e){alert(e)}}
...
async function loadTF(tf){
  ...
  try{
    ...
    for(let [name, info] of Object.entries(d.coins)){
      ...
      html+=`<div class=coin-row onclick="openDetails('${name}')"><div style="display:flex;gap:12px;align-items:center"><div class="coin-icon ${icon}">${ico}</div><div><b>${name}</b><div style="font-size:12px;color:#64748b">RSI ${info.rsi} • ${info.trend}</div><div style="font-size:11px;color:#94a3b8">Affidabilità ${info.conf}%</div></div></div><div style="text-align:right"><span class="badge ${bgFor(info.signal)}">${info.signal} ${info.conf}%</span><div style="font-weight:800;margin-top:4px">€${info.price.toFixed(2)}</div><div style="font-size:11px;color:#94a3b8">TAP per dettagli ›</div></div></div>`;
    }
    ...
    if('serviceWorker' in navigator){
      ...
      if(reg){const s=await reg.pushManager.getSubscription(); if(s){document.getElementById('subStatus').innerText='Push: ATTIVO [SERVER] (già iscritto)';}}
    }
...
async function loadHistGlobal(){
  try{
    ...
    if(!list.length){c.innerHTML='<div style="padding:10px;color:#94a3b8;font-size:13px">Nessun segnale >60% ancora su 1H/4H.</div>'; return;}
...
async function openDetails(coin){
  ...
  document.getElementById('mExplain').innerText= info.signal=='VENDI'?`RSI ${info.rsi} ipercomprato (${info.trend}). Affidabilità ${info.conf}%` : info.signal=='COMPRA'?`RSI ${info.rsi} ipervenduto. Affidabilità ${info.conf}%` : `RSI ${info.rsi} neutro (${info.trend}). Laterale.`;
  document.getElementById('modal').classList.add('show');
  try{
    const r=await fetch('/api/history?coin='+coin+'&min_conf=60'); const list=await r.json();
    document.getElementById('mHist').innerHTML = list.length ? list.slice(0,8).map(h=>`<div class=hist-item><div><b>${h.tf}</b> ${h.signal} ${h.conf}%</div><div>${h.time} €${h.price.toFixed(0)}</div></div>`).join('') : '<div style="font-size:12px;color:#94a3b8">Nessuno storico >60%</div>';
  }catch{}
}
