...
ALL_TFS = ["5m","1H","4H","1D"]
VERSION = "V55 - CONFLUENZA + PUSH INTELLIGENTI + FIX 52%"

# Per push intelligente - memoria temporanea ultimo invio
LAST_PUSH = {}  # key = coin_TF -> timestamp
...
def analyze_coin(name, tf):
    ...
    if not ohlc or len(ohlc)<20:
        ...
        # FIX 52%: invece di ASPETTA 52% fake, ritorna stato LOADING
        return {
            ...
            "quality_score":0,"quality_simple":f"Dati {tf} in aggiornamento da {source}/{ohlc_src} - Binance bloccato, provo Kraken...",
    ...
    if conf>=65 and st_trend==1 and close_price>ema50:
        ...
        quality_simple=f"[{source}/{ohlc_src}] LIVE ${price:.2f} - RSI{int(rsi)} EMA{int(ema50)}/{int(ema200)} ST{'UP' if st_trend==1 else 'DOWN'} Stoch{stoch_k} = {conf}% TF {tf}"
...
def api_confluence():
    """Ritorna allineamento multi-TF per ogni coin"""
    out={}
    for name in PAIRS_LIVE.keys():
        scores=[]
        signals=[]
...
@app.route("/api/push_check")
def api_push_check():
    """Check se dobbiamo mandare push intelligente: solo ENTRA >=80% e cooldown 2h"""
    tf=request.args.get("tf","1H")
    now=time.time()
    to_push=[]
    for name in PAIRS_LIVE.keys():
        ...
        if not d: continue
        key=f"{name}_{tf}"
        last=LAST_PUSH.get(key,0)
        cooldown=7200  # 2 ore
        if d["quality_color"]=="entra" and d["conf"]>=80 and (now-last)>cooldown:
            to_push.append({"coin":name,"tf":tf,"signal":d["signal"],"conf":d["conf"],"price":d["price"]})
            LAST_PUSH[key]=now
    return jsonify({"ok":True,"push":to_push,"time":rome_now().isoformat()})
...
.badge{padding:4px 10px;border-radius:20px;font-size:11px;font-weight:700;white-space:nowrap;display:inline-block;line-height:1.2}
...
.conf-row{display:flex;gap:4px;margin-top:4px}
.conf-dot{width:8px;height:8px;border-radius:50%;display:inline-block}
.conf-dot.compra{background:#22c55e}
.conf-dot.vendi{background:#ef4444}
.conf-dot.aspetta{background:#cbd5e1}
.conf-dot.loading{background:#fde68a}
...
.loading-big{background:#fef9c3;border:2px dashed #eab308}
...
.stat-card b{display:block;font-size:16px;color:#0f172a}
.stat-card span{font-size:10px;color:#64748b}
...
.conf-banner.forte-compra{background:#dcfce7;color:#166534;border:1px solid #86efac}
.conf-banner.compra{background:#fef3c7;color:#92400e}
.conf-banner.forte-vendi{background:#fee2e2;color:#991b1b;border:1px solid #fecaca}
.conf-banner.vendi{background:#ffedd5;color:#9a3412}
.conf-banner.neutro{background:#f1f5f9;color:#475569}
</style>
</head>
<body>
<div class="header">
...
<div style="flex:1">
<div style="font-weight:800;font-size:16px">VENDI - <span style="background:#22c55e;color:#052e16;padding:2px 6px;border-radius:6px;font-size:12px">V55</span> CONFLUENZA</div>
<div style="font-size:11px;opacity:0.8">Multi-TF + Push intelligenti + Fix 52%</div>
</div>
...
<div id="mExpert" style="font-size:9px;color:#94a3b8;text-align:center;margin-top:8px"></div>
</div>
</div>
...
function confDots(details){
  var tfs=['5m','1H','4H','1D'];
  return tfs.map(tf=>{
    var s=(details && details[tf]) || 'ASPETTA';
    var cls=s=='COMPRA'?'compra':s=='VENDI'?'vendi':s=='LOADING'?'loading':'aspetta';
    return `<span class="conf-dot ${cls}" title="${tf}:${s}"></span>`;
  }).join('');
}
...
async function loadTF(tf){
  ...
  document.getElementById('coins').innerHTML='<div style="padding:20px;text-align:center">Carico V55 '+tf+' con confluenza...</div>';
  try{
    var res=await fetch('/api/signals?tf='+tf);
    var d=await res.json();
    lastData=d;
    // carica anche confluenza per banner
    ...
    for(var name in d.coins){
      ...
      var cInfo=confData.confluence[name];
      var dots=confDots(cInfo?cInfo.details:null);
      var spark=sparklineSVG(info.spark);
      var actionText=info.quality_color=='loading'?'Aggiornamento dati': info.quality_color=='entra' ? (info.signal=='COMPRA'?'Compra ora':'Vendi ora') : info.quality_color=='quasi' ? 'Quasi pronto' : 'Non fare nulla';
      html+=`<div class="coin-row" onclick="openDetails('${name}')"><div style="display:flex;gap:10px;align-items:center"><div class="coin-icon ${iconClass}">${ico}</div><div><b style="font-size:16px">${name}</b> - ${price}${spark}<div style="font-size:11px;color:#64748b;margin-top:2px;display:flex;align-items:center;gap:4px">${actionText} <span class="conf-row">${dots}</span> <span style="font-size:9px">${cInfo?cInfo.confluence:''}</span></div></div></div><div style="text-align:right">${qBadge}<div style="font-size:11px;color:#64748b;margin-top:4px">${info.signal} ${info.conf>0?info.conf+'%':''}</div></div></div>`;
    }
...
async function loadConfluence(){
  document.getElementById('coins').innerHTML='<div style="padding:20px;text-align:center">Analizzo tutti i timeframe...</div>';
  ...
  document.getElementById('coins').innerHTML=html;
  document.getElementById('confBanner').style.display='none';
}
...
async function openDetails(coin){
  ...
  document.getElementById('mConfluence').innerHTML=cInfo?`<b>Confluenza multi-TF:</b> ${cInfo.confluence}<br><span style="font-size:11px">5m:${cInfo.details['5m']} | 1H:${cInfo.details['1H']} | 4H:${cInfo.details['4H']} | 1D:${cInfo.details['1D']} | Media ${cInfo.avg}%</span>`:'Confluenza in caricamento...';
  ...
  document.getElementById('mExpert').textContent='Fonte: '+info.source+'/'+info.ohlc_src+' - Aggiornato ogni 20s - V55 Confluenza';
  document.getElementById('modal').classList.add('show');
}
function closeModal(){ document.getElementById('modal').classList.remove('show'); }
function togglePush(){
  if(Notification.permission!=='granted'){ Notification.requestPermission(); alert('Attiva notifiche e ricarica'); return; }
  alert('Push intelligenti attivi: ti avviso solo se ENTRA >=80% e solo ogni 2 ore per stessa coin/TF');
}

loadTF('1H');
setInterval(()=>loadTF(curTF), 20000);
setInterval(async ()=>{
  try{
    var r=await fetch('/api/push_check?tf='+curTF);
    var j=await r.json();
    if(j.push && j.push.length>0 && Notification.permission==='granted'){
      j.push.forEach(p=>{
        new Notification(`VENDI ${p.coin} ${p.signal} ${p.conf}%`,{body:`Prezzo $${p.price.toFixed(2)} TF ${p.tf} - Confluenza forte`});
      });
    }
  }catch(e){}
}, 60000);
