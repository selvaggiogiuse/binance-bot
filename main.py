...
<div class="header"><div class="h-row"><div class="logo">V€</div><div style="flex:1"><div style="font-weight:900;font-size:14px">Vendi STABILE FIX</div><div style="font-size:10px;opacity:.6">BTC • ETH • ORO | Fix PAXG | 5M+ no 1M</div></div><button class="btn btn-icon" id="themeBtn">🌙</button><button class="btn btn-icon" id="notifBtn">🔔</button><button class="btn btn-icon" id="soundBtn">🔊</button><button class="btn btn-icon" id="histBtn">📜</button></div>
...
// CONFIG CON FALLBACK - risolve il problema PAXG
const COIN_CONFIG=[
...
async function getEurUsdtRate(){
  try{
    let kl=await fetchKlines('EURUSDT','1h',2);
    if(kl.length) return parseFloat(kl[kl.length-1][4]);
  }catch{}
  return 1.08;
}
async function loadCoinForTF(cfg, tf){
 try{
  ...
  for(let sym of cfg.fallbacks){
    kl=await fetchKlines(sym,tf,150);
    if(kl && kl.length){usedSymbol=sym;break;}
  }
  if(!kl || !kl.length) return null;
  let closes=kl.map(c=>parseFloat(c[4]));let volumes=kl.map(c=>parseFloat(c[5]));
  // Se usiamo USDT per PAXG, convertiamo in EUR
  if(usedSymbol && usedSymbol.endsWith('USDT')){
    let rate=await getEurUsdtRate();
    closes=closes.map(p=>p/rate);
    kl=kl.map(c=>{let nc=[...c]; nc[1]=(parseFloat(c[1])/rate).toString(); nc[2]=(parseFloat(c[2])/rate).toString(); nc[3]=(parseFloat(c[3])/rate).toString(); nc[4]=(parseFloat(c[4])/rate).toString(); return nc;});
  }
 ...
 }catch(e){console.log('err',cfg.id,e);return null;}
}
...
async function refresh(){
 ...
 document.getElementById('upd').textContent='Agg: '+new Date().toLocaleTimeString('it-IT')+' • TF:'+currentTF.toUpperCase()+' • Scan 5M-1D (no 1M) | BTC ETH ORO (fix)';
...
@app.route("/")
def home():
    return '<a href="/app">Vai alla versione FIX - BTC ETH ORO</a>'
