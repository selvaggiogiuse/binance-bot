...
VERSION = "V58 PRO FIX - 3 API BACKUP"
LAST_TELEGRAM = {}
...
def send_telegram_signal(coin, tf, signal, conf, price, rsi, stoch, sl, tp, sl_pct, tp_pct, source, extra_info=""):
    ...
    if DAILY_STATS.get("blocked"):
        return {"ok": False, "error": "Daily STOP"}
    if DAILY_STATS.get("trades", 0) >= 5:
        return {"ok": False, "error": "Max 5 trade/giorno"}
    if DAILY_STATS.get("loss_streak", 0) >= 2:
        return {"ok": False, "error": "Stop 2 loss"}
    ...
    text = f"""{emoji} *{signal} {coin} {conf}%* {tf_emoji} {tf} - V58 PRO FIX
...
⏰ {rome_now().strftime('%H:%M:%S %d/%m')} Rome"""
    ...
    try:
        ...
        if r.status_code==200:
            ...
            TRADE_HISTORY.append({"time": rome_now().isoformat(),"coin": coin,"tf": tf,"signal": signal,"conf": conf,"price": price,"sl": sl,"tp": tp,"sl_pct": sl_pct,"tp_pct": tp_pct,"rsi": rsi,"qty": qty,"risk": risk_amount})
            ...
            save_json_file(DAILY_STATS_FILE, DAILY_STATS)
            print(f"[{rome_now()}] TELEGRAM PRO INVIATO {coin} {conf}%")
            return {"ok": True, "rr": rr, "qty": qty}
        else:
            return {"ok": False, "error": f"TG API {r.status_code}"}
    except Exception as e:
...
# --- FIX: 3 API DI BACKUP PER PREZZO LIVE ---
def get_live_price_ticker(name):
    # 1) Binance
    symbol_map = {"BTC": "BTCUSDT", "ETH": "ETHUSDT", "ORO": "PAXGUSDT"}
    symbol = symbol_map.get(name, "BTCUSDT")
    try:
    ...
    # 3) CoinGecko (fallback finale, funziona sempre)
    try:
        cg_map={"BTC":"bitcoin","ETH":"ethereum","ORO":"pax-gold"}
        cg_id=cg_map.get(name,"bitcoin")
        r=requests.get(f"https://api.coingecko.com/api/v3/simple/price?ids={cg_id}&vs_currencies=usd",timeout=4)
        if r.status_code==200:
            j=r.json()
            if cg_id in j and "usd" in j[cg_id]:
                return float(j[cg_id]["usd"]), f"COINGECKO:{cg_id}"
    except: pass
    return None, "FAIL"
...
def analyze_coin_pro(name, tf="5m", send_telegram=False):
    ohlc_5m, src_5m = fetch_ohlc_with_fallback(name, tf, 200)
    ohlc_1h, src_1h = fetch_ohlc_with_fallback(name, "1h", 100)
    live_price, live_src = get_live_price_ticker(name)

    # Se anche live fallisce, prova a usare ultimo close disponibile
    if not ohlc_5m or len(ohlc_5m)<20:
        if live_price is None:
            # Ultimo tentativo: usa trade_history o prezzo fisso per non mostrare 0
            if TRADE_HISTORY:
                last_for_coin = [t for t in TRADE_HISTORY if t["coin"]==name]
                if last_for_coin:
                    live_price = last_for_coin[-1]["price"]
                    live_src = "CACHE:HISTORY"
        if live_price is None:
            return None, None
    ...
    if ohlc_1h and len(ohlc_1h)>=21:
        ...
        h1_trend_text = f"1H {'UP' if h1_up else 'DOWN'} RSI{int(rsi_1h)}"
    else:
    ...
    if conf>=70 and vol_ratio>=0.7:
        quality_color="entra"; quality_label="ENTRA"
    elif conf>=60:
        quality_color="quasi"; quality_label="QUASI PRONTO"
    else:
        quality_color="wait"; quality_label="ASPETTA"
        signal="ASPETTA"
        sl=price*0.992; tp=price*1.008; sl_pct=0.8; tp_pct=1.5

    mtf_block=False
    if ohlc_1h and len(ohlc_1h)>=21:
        closes_1h=[c["close"] for c in ohlc_1h]
        ema21_1h=ema_calc(closes_1h,21)
        if signal=="COMPRA" and closes_1h[-1] < ema21_1h and conf<80:
            quality_color="quasi"; quality_label="QUASI - 1H CONTRO"; mtf_block=True
        if signal=="VENDI" and closes_1h[-1] > ema21_1h and conf<80:
            quality_color="quasi"; quality_label="QUASI - 1H CONTRO"; mtf_block=True

    extra_info = f"🔍 {h1_trend_text} | Vol x{vol_ratio:.1f} {'✅' if vol_ratio>=0.9 else '⚠️'} | {source}"
    if mtf_block:
        extra_info += " | ⛔ MTF BLOCCO"
    ...
    data={
        ...
        "quality_score":int(conf),"quality_simple":f"PRO {tf} {signal} {conf}% {h1_trend_text}",
...
def api_signals():
    ...
    for name in PAIRS_LIVE.keys():
        ...
        if data is None:
            # Fallback finale per non mostrare 0
            data={"price":0,"real_price":0,"close_price":0,"source":"FAIL","signal":"LOADING","conf":0,"quality_color":"loading","quality_label":"RIPROVO...","quality_score":0,"quality_simple":"Binance bloccato, riprovo Kraken/CoinGecko","rsi":50,"rsi_fast":50,"ema9":0,"ema21":0,"st_trend":0,"stoch_k":50,"vwap":0,"support":0,"resistance":0,"vol_ratio":1,"h1_trend":"N/A","sl":0,"tp":0,"sl_pct":0.8,"tp_pct":1.5,"spark":[],"extra":"API Binance bloccata su Render, uso backup","mtf_block":False}
        result[name]=data
...
def api_stats():
    reset_daily_if_needed()
    return jsonify({"version":VERSION,"total_signals":len(TRADE_HISTORY),"daily":DAILY_STATS,"last_telegram":LAST_TELEGRAM,"capital":CAPITAL,"risk_pct":RISK_PCT,"threshold":TELEGRAM_MIN_CONF})

@app.route("/api/telegram_test")
def api_telegram_test():
    if not TELEGRAM_ENABLED:
        return jsonify({"ok":False,"error":"Configura ENV"}), 400
    res=send_telegram_signal("BTC","5m","COMPRA",80,67000,58,55,66500,68000,0.7,1.5,"TEST","Test V58 PRO FIX - 3 API backup ✅")
    return jsonify(res)
...
def api_force_telegram():
    ...
    for name in PAIRS_LIVE.keys():
        ...
        res=send_telegram_signal(name,"5m","COMPRA",75,price,56,54,price*0.993,price*1.013,0.7,1.3,"FORCE","Force V58 PRO FIX",)
        results[name]=res
...
def app_page():
    if not check_auth():
        return Response(f"""<html><body style="background:#020617;color:white;font-family:sans-serif;display:flex;align-items:center;justify-content:center;height:100vh"><form><h2>🔒 V58 PRO Protetta</h2><p>Inserisci password</p><input type="password" name="pwd" style="padding:10px;border-radius:8px;border:none;width:250px"><button style="padding:10px 15px;border-radius:8px;background:#22c55e;border:none;font-weight:bold;margin-left:8px">Entra</button></form></body></html>""", mimetype="text/html")
    html = """<!DOCTYPE html>
<html lang="it"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>VENDI V58 PRO REAL MONEY</title>
<style>*{box-sizing:border-box;font-family:Inter,system-ui,sans-serif}body{margin:0;background:#020617;color:#f1f5f9}.header{background:linear-gradient(135deg,#020617,#1e293b);padding:12px 16px;display:flex;align-items:center;gap:12px;position:sticky;top:0;z-index:10;border-bottom:1px solid #1e293b}.logo{width:48px;height:48px;border-radius:12px;background:linear-gradient(135deg,#22c55e,#16a34a);display:flex;align-items:center;justify-content:center;font-weight:900;color:#052e16}.badge{padding:4px 10px;border-radius:20px;font-size:11px;font-weight:800}.badge-entra{background:#22c55e;color:#052e16;animation:glow 1s infinite alternate}.badge-quasi{background:#facc15;color:#422006}.badge-wait{background:#1e293b;color:#94a3b8}@keyframes glow{0%{box-shadow:0 0 5px #22c55e}100%{box-shadow:0 0 15px #22c55e}}.tfs{display:flex;gap:6px;padding:10px 12px;overflow-x:auto;background:#0f172a}.tfs button{border:1px solid #1e293b;background:#1e293b;color:#cbd5e1;padding:8px 14px;border-radius:20px;font-weight:700;cursor:pointer;font-size:13px}.tfs button.active{background:#22c55e;color:#052e16}.coin-row{display:flex;justify-content:space-between;align-items:center;padding:14px;border-bottom:1px solid #1e293b;background:#0f172a;cursor:pointer}.coin-icon{width:38px;height:38px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-weight:900;color:white}.coin-icon.btc{background:#f7931a}.coin-icon.eth{background:#8b5cf6}.coin-icon.oro{background:#ca8a04}.stats-bar{display:flex;gap:8px;margin:8px 12px}.stat-mini{background:#1e293b;border:1px solid #334155;border-radius:10px;padding:8px 10px;flex:1;text-align:center}.stat-mini b{display:block;font-size:13px}.stat-mini span{font-size:9px;color:#94a3b8}.tele-banner{margin:8px 12px;padding:10px 12px;border-radius:10px;font-size:12px}.tele-on{background:#052e16;border:1px solid #16a34a;color:#86efac}.tele-off{background:#450a0a;border:1px solid #dc2626;color:#fca5a5}.modal{position:fixed;inset:0;background:rgba(0,0,0,0.7);display:none;align-items:flex-end;justify-content:center;z-index:50}.modal.show{display:flex}.modal-box{background:#0f172a;width:100%;max-width:500px;border-radius:20px 20px 0 0;padding:20px;max-height:92vh;overflow:auto;border:1px solid #1e293b}.big-box{border-radius:14px;padding:16px;margin:12px 0;text-align:center}.entra-big{background:#052e16;border:2px solid #22c55e}.quasi-big{background:#422006;border:2px solid #facc15}.wait-big{background:#1e293b;border:1px solid #334155}</style></head><body>
<div class="header"><div class="logo">V58</div><div style="flex:1"><div style="font-weight:800">VENDI V58 PRO <span style="background:#22c55e;color:#052e16;padding:2px 6px;border-radius:6px;font-size:10px">FIX REAL MONEY</span></div><div style="font-size:10px;opacity:0.7">MTF 5m+1H • SL dinamico • 3 API Backup • Risk 1%</div></div><div><button onclick="testTelegram()" style="background:#0088cc;color:white;border:none;padding:6px 10px;border-radius:20px;font-size:11px;font-weight:700">📱 Test</button></div></div>
...
<div id="coins" style="background:#0f172a;border-radius:12px;margin:0 8px;overflow:hidden;border:1px solid #1e293b;min-height:100px"><div style="padding:20px;text-align:center;color:#94a3b8">Carico V58 PRO FIX...</div></div>
...
<script>var curTF='5m';var lastData=null;function qualityBadge(info){var c=info.quality_color||'wait';var l=info.quality_label||'ASPETTA';if(c=='entra')return '<span class="badge badge-entra">'+l+'</span>';if(c=='quasi')return '<span class="badge badge-quasi">'+l+'</span>';return '<span class="badge badge-wait">'+l+'</span>';}
...
async function sendThisToTelegram(){if(!window._currentCoin)return;try{var r=await fetch(`/api/signals?tf=${curTF}&telegram=1`);var j=await r.json();alert('Risultato: '+JSON.stringify(j.telegram_results[window._currentCoin]||'Nessun ENTRA'));}catch(e){alert(e.message);}}
...
def background_loop():
    print(f"V58 PRO FIX loop started - Cap {CAPITAL} Risk {RISK_PCT}%")
    while True:
        try:
            reset_daily_if_needed()
